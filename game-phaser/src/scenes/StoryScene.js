// ===== StoryScene：HD-2D 剧情/对话/选择系统 =====

import Phaser from 'phaser';
import { GAME_WIDTH, GAME_HEIGHT } from '../config/gameConfig.js';
import { COLORS, FONTS, MSG_STYLES, GLASS } from '../config/theme.js';
...
export class StoryScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.postFX.applyPreset('story');

    // 角色立绘区域（左/右）
    this.leftPortrait = this.add.sprite(160, 260, '__DEFAULT').setScale(2).setVisible(false);
    this.rightPortrait = this.add.sprite(GAME_WIDTH - 160, 260, '__DEFAULT').setScale(2).setVisible(false);
  ...
  createDialogueBox() {
    // 毛玻璃对话框背景
    const dlgW = GAME_WIDTH - 40;
    const dlgH = 140;

    // 使用预生成纹理（如果存在）
    if (this.textures.exists('dialogue_bg')) {
      this.dlgBg = this.add.image(GAME_WIDTH / 2, 80, 'dialogue_bg')
        .setDisplaySize(dlgW + 40, dlgH + 20);
      this.dialogueContainer.add(this.dlgBg);
    } else {
      // 毛玻璃渐变背景
      const bg = this.add.graphics();
      bg.fillStyle(GLASS.fillBottom, 0.88);
      bg.fillRect(20, 10, dlgW, dlgH);
      bg.fillStyle(GLASS.fillTop, 0.4);
      bg.fillRect(20, 10, dlgW, dlgH / 2);
      // 顶部金色高光
      bg.lineStyle(1, GLASS.borderGradientTop, 0.3);
      bg.beginPath();
      bg.moveTo(24, 10);
      bg.lineTo(20 + dlgW - 4, 10);
      bg.strokePath();
      // 边框
      bg.lineStyle(1, GLASS.borderGradientBottom, 0.4);
      bg.strokeRect(20, 10, dlgW, dlgH);
      this.dlgBg = bg;
    ...
    if (this.textures.exists('glass_noise')) {
      const noise = this.add.tileSprite(GAME_WIDTH / 2, 80, dlgW, dlgH, 'glass_noise')
        .setAlpha(0.03);
      this.dialogueContainer.add(noise);
    }

    // 说话人名字（带底部装饰线）
    this.speakerText = this.add.text(45, 18, '', {
    ...
    // 对话正文
    this.dialogueText = this.add.text(45, 44, '', {
      fontFamily: FONTS.main, fontSize: FONTS.size.md, color: COLORS.textMain,
      wordWrap: { width: GAME_WIDTH - 110 }, lineSpacing: 6
    });
    this.dialogueContainer.add(this.dialogueText);

    // 继续提示（金色三角 带发光）
    this.continueHint = this.add.text(GAME_WIDTH - 60, 132, '▼', {
    ...
    // 点击区域（覆盖整个对话框）
    const clickZone = this.add.rectangle(GAME_WIDTH / 2, 80, dlgW, dlgH, 0x000000, 0)
      .setInteractive();
    this.dialogueContainer.add(clickZone);
    clickZone.on('pointerdown', () => this.onDialogueClick());
  }
  ...
  showDialogue(node) {
    ...
    // 显示角色立绘
    const customMainCharKey = 'player_idle_stand';
    const spriteKey = `char_${node.speaker}`;
    if (node.speaker === 'chen_dao' && this.textures.exists(customMainCharKey)) {
      const source = this.textures.get(customMainCharKey).getSourceImage();
      const maxSide = Math.max(source.width || 128, source.height || 128);
      const scale = 180 / maxSide;
      this.leftPortrait.setTexture(customMainCharKey, 0).setScale(scale).setVisible(true);
    } else if (this.textures.exists(spriteKey)) {
      this.leftPortrait.setTexture(spriteKey, 0).setScale(2).setVisible(true);
  ...
  showChoice(node) {
    ...
    node.choices.forEach((choice, i) => {
      ...
      const btn = GlassPanel.createButton(
        this, GAME_WIDTH / 2, y, GAME_WIDTH - 120, 40,
        choice.text,
        () => {
          if (choice.effects) {
        ...
        },
        {
          ...
          hoverColor: meetsReq ? COLORS.goldStr : '#555555',
        }
      );
  ...
  showSystemPopup(node) {
    ...
    const panel = GlassPanel.create(this, cx, cy, 500, panelH, {
      accentColor: COLORS.systemGreen,
      accentStr: COLORS.systemGreenStr,
    });
    this.systemContainer.add(panel.container);
    ...
    const confirmBtn = GlassPanel.createButton(
      this, cx, btnY, 120, 30, '确  认',
      () => {
        this.systemContainer.setVisible(false);
        if (node.next) this.loadNode(node.next);
      },
      { color: COLORS.systemGreenStr, hoverColor: '#88ffcc' }
    );
    this.systemContainer.add(confirmBtn);
