...
export class InventoryScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.tabs.forEach((tab, i) => {
      ...
      const txt = this.add.text(x, y, this.tabLabels[i], {
        ...
        color: isActive ? COLORS.goldStr : COLORS.textDim
      }).setOrigin(0.5).setInteractive({ useHandCursor: true });

      txt.on('pointerdown', () => this.switchTab(tab));
      txt.on('pointerover', () => { if (tab !== this.currentTab) txt.setColor('#b08830'); });
      txt.on('pointerout', () => { if (tab !== this.currentTab) txt.setColor(COLORS.textDim); });
  ...
  drawTabLine() {
    this.tabLine.clear();
    const activeIdx = this.tabs.indexOf(this.currentTab);
    if (activeIdx >= 0) {
      const cx = GAME_WIDTH / 2;
      const panelW = 550;
      const panelH = 420;
      const x = cx - panelW / 2 + 60 + activeIdx * 130;
      const y = GAME_HEIGHT / 2 - panelH / 2 + 60;
      this.tabLine.lineStyle(2, COLORS.gold, 0.6);
      this.tabLine.beginPath();
      this.tabLine.moveTo(x - 25, y);
      this.tabLine.lineTo(x + 25, y);
      this.tabLine.strokePath();
    }
  }
